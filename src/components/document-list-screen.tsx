...
import { DOCUMENTS, type DocumentCategory } from '@/constants/documents';
...
type DocumentListScreenProps = {
  category: DocumentCategory;
};

export function DocumentListScreen({ category }: DocumentListScreenProps) {
  const { t } = useTranslation();
  const [query, setQuery] = useState('');
  const uploads = useDocumentUploads();
  const { isLoading, isSyncing } = useDocumentUploadsSyncStatus();
  const translationKey = category === 'id' ? 'ids' : 'documents';
  ...
  return (
    <SafeAreaView style={styles.container}>
      <ScreenHeader title={<Text style={styles.headerTitle}>{t(`${translationKey}.title`)}</Text>} />
      ...
      <View style={styles.scrollContainer}>
        {isLoading ? (
          <View style={styles.loadingState}>
            ...
            <Text style={styles.loadingText}>{t(`${translationKey}.loading`)}</Text>
          </View>
        ) : (
          <ScrollView
            ...
            {filteredDocuments.length === 0 ? (
              <Text style={styles.emptyState}>{t(`${translationKey}.empty`)}</Text>
            ) : (
